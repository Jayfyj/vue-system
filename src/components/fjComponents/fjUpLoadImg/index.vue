<template>
    <div>
        ...
        <template v-if="!draggable">
          <div
              ...
              <div class="file-list-content">
                <img :src=" downloadUrl + item.pictureId" style="width=100px;height:100px;margin-right:20px"/>
                <a
                    class="file-btn"
                    download
                    :href="downloadUrl+item.pictureId"
                    target="_blank"
                >下载</a>
                <span
                    class="file-btn-del"
        ...
        <template v-else>
          <vuedraggable class="wrapper" v-model="pictureList" v-if="pictureList.length">
            <transition-group>
                <div
                  class="file-list"
                  v-for="(item, index) in pictureList"
                  ...
                  <div class="file-list-content">
                    ...
                    <gmd-icon type="down-circle"  class="down-btn" @click="down(index)" v-if="index!==pictureList.length-1"/>
                  </div>
                </div>
            </transition-group>
          </vuedraggable>
  ...
  methods: {
    ...
    handleFileChange(e) {
      ...
      if(this.acceptFile.indexOf(file.name.split('.')[1])<0){
        ...
        // if (file.size > limit) {
        //   this.$message.error(`上传${this.upLoadFileName}大小不能超过300KB`, 3);
        //   return;
        // }
    ...
    up (i) {
      ...
      [this.pictureList[i-1],this.pictureList[i]] = [this.pictureList[i],this.pictureList[i-1]]

      this.$forceUpdate()
    },

    down (i) {
      if(i >= this.pictureList.length - 1) return
